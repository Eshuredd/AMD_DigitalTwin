...
DEFAULT_SOIL_PARAMETER_BASIS = "mvp_assumed_volumetric_field_capacity_wilting_point_by_soil_texture"
...
def validate_soil_water_params(params: SoilWaterParams) -> None:
    ...
    field_capacity = _validate_finite_positive_number("field_capacity", params.field_capacity)
    wilting_point = _validate_finite_positive_number("wilting_point", params.wilting_point)
...
def get_soil_water_params(
    ...
    params_by_texture = params_by_texture if params_by_texture is not None else DEFAULT_SOIL_WATER_PARAMS_BY_TEXTURE
    if not isinstance(params_by_texture, dict):
        raise ValueError("params_by_texture must be a dict of SoilTexture to SoilWaterParams.")

    texture_keys = set(params_by_texture.keys())
    expected_keys = set(SoilTexture)
    missing = expected_keys - texture_keys
    extra = texture_keys - expected_keys
    if missing or extra:
        raise ValueError(
            f"params_by_texture must contain exactly all SoilTexture keys; missing={missing}, extra={extra}."
        )

    for texture, params in params_by_texture.items():
        if not isinstance(texture, SoilTexture):
            raise ValueError("All params_by_texture keys must be SoilTexture enum members.")
        validate_soil_water_params(params)

    return params_by_texture[soil_texture]


def validate_root_depth_by_stage(root_depth_by_stage: dict[GrowthStage, float]) -> None:
    """Validate root depth configuration keyed by GrowthStage."""
    if not isinstance(root_depth_by_stage, dict):
        raise ValueError("root_depth_by_stage must be a dict of GrowthStage to float values.")
    ...
    if missing or extra:
        raise ValueError(
            f"root_depth_by_stage must contain exactly the tomato GrowthStage keys; missing={missing}, extra={extra}."
        )

    for stage, value in root_depth_by_stage.items():
        if not isinstance(stage, GrowthStage):
            raise ValueError("All root_depth_by_stage keys must be GrowthStage enum members.")
        if not _is_finite_number(value):
            raise ValueError(f"Root depth for {stage.value} must be a finite number.")
        if float(value) <= 0.0:
            raise ValueError(f"Root depth for {stage.value} must be greater than 0.")
...
def get_root_depth_m(
    ...
    config = root_depth_by_stage if root_depth_by_stage is not None else DEFAULT_ROOT_DEPTH_M_BY_STAGE
    validate_root_depth_by_stage(config)
    return float(config[growth_stage])


def compute_taw_mm(*, field_capacity: float, wilting_point: float, root_depth_m: float) -> float:
    """Compute total available water (TAW) in millimeters."""
    field_capacity = _validate_finite_positive_number("field_capacity", field_capacity)
    wilting_point = _validate_finite_positive_number("wilting_point", wilting_point)
    root_depth_m = _validate_finite_positive_number("root_depth_m", root_depth_m)
    if field_capacity <= wilting_point:
        raise ValueError("field_capacity must be greater than wilting_point.")
    return 1000.0 * (field_capacity - wilting_point) * root_depth_m


def compute_raw_threshold_mm(*, taw_mm: float, p_allowable: float) -> float:
    ...
    if not _is_finite_number(p_allowable):
        raise ValueError("p_allowable must be a finite number.")
    p = float(p_allowable)
    if p < 0.0 or p > 1.0:
...
def update_root_zone_depletion_mm(
    *,
    ...
    prev = 0.0 if previous_root_zone_depletion_mm is None else _validate_finite_non_negative_number(
    ...
    irrigation = _validate_finite_non_negative_number("irrigation_mm", irrigation_mm)
...
def classify_moisture_state(*, root_zone_depletion_mm: float, raw_threshold_mm: float) -> MoistureState:
    """Classify the moisture state of the root zone."""
    depletion = _validate_finite_non_negative_number("root_zone_depletion_mm", root_zone_depletion_mm)
    raw_threshold = _validate_finite_non_negative_number("raw_threshold_mm", raw_threshold_mm)
...
def classify_stress_band(*, root_zone_depletion_mm: float, raw_threshold_mm: float) -> StressBand:
...
def extract_irrigation_amount_mm(
    ...
    if not isinstance(last_irrigation_event, LastIrrigationEvent):
        raise ValueError("last_irrigation_event must be a LastIrrigationEvent instance.")
    amount_mm = _validate_finite_non_negative_number("last_irrigation_event.amount_mm", last_irrigation_event.amount_mm)
...
def compute_water_state(
    *,
    ...
    latitude = _validate_finite_number(latitude_deg)
    if latitude < -90.0 or latitude > 90.0:
        raise ValueError("latitude_deg must be between -90 and 90 inclusive.")
    elevation = _validate_finite_number(elevation_m)
    p_allowable_value = _validate_finite_non_negative_number("p_allowable", p_allowable)
    if p_allowable_value > 1.0:
        raise ValueError("p_allowable must be between 0.0 and 1.0 inclusive.")
    ...
    eto_computed = _validate_finite_non_negative_number("eto_value", eto_result["eto_value"])
    ...
    raw_threshold = compute_raw_threshold_mm(taw_mm=taw, p_allowable=p_allowable_value)
    irrigation_mm = extract_irrigation_amount_mm(last_irrigation_event, current_date)
    ...
    computed_at_value = computed_at if computed_at is not None else datetime.now(timezone.utc)
